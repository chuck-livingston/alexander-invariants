#     Alexander invariants for knots.
...
#     module, and its primary and invariant-factor decompositions.
#
#     Programming assistance provided by Claude.ai
#
#     Usage:
#       In SageMath, run:
...
#
#     Parameters:
#         code    -- a PD code, DT code, or alphabetical DT code
#         decomp  -- 'alex_polys_decomp' (default), 'primary_decomp',
#                    or 'invariant_factor_decomp'
#         notation -- 'PD' (default), 'DT', or 'DT_alphabetical'
#
#     Example:
...
#         alexander_invariants([4,6,2], notation='DT')
#         alexander_invariants('bca', notation='DT_alphabetical')
...
class UnderdeterminedDecompositionError(ValueError):
    """
    Raised when the primary decomposition of the Alexander module cannot
    be uniquely determined from the Fox matrix alone.
    """
    pass
...
def _check_pd_code(pd_code):
    ...
    for x in pd_code:
        if len(x) != 4:
            raise ValueError(
                f"Invalid PD code: each entry must have exactly 4 elements, "
                f"but got {x} (length {len(x)})"
            )

    N = 2 * len(pd_code)
    for x in pd_code:
        cond_c = (x[2] - x[0]) % N == 1
        cond_d = (x[3] - x[1]) % N in (1, N - 1)
        if not (cond_c and cond_d):
            raise ValueError(
                f"Invalid PD code: crossing orientation condition failed at entry {list(x)}"
            )
    ...
    for pdc in pd_code:
        for x in pdc:
            counts[x % N] += 1
    bad = [i for i, c in enumerate(counts) if c != 2]
    if bad:
        raise ValueError(
            f"Invalid PD code: arc label(s) {bad} do not appear exactly twice"
        )

    return True


def alexander_invariants(code, decomp = 'alex_polys_decomp', notation = 'PD'):
    """
    ...
    if notation == 'PD':
        pd_code = code
    if notation == 'DT':
        pd_code = PD_from_dt(code)
    if notation == 'DT_alphabetical':
        pd_code = PD_from_dt_alphabetical(code)

    try:
        _check_pd_code(pd_code)
    except ValueError as e:
    ...
    except UnderdeterminedDecompositionError:
        raise UnderdeterminedDecompositionError(
            "The Alexander module of this knot cannot be uniquely determined "
            "by the program."
        )  from None
    ...
    def alex_polys(pd, decomp='alex_polys_decomp'):
        ...
        Parameters:
            ...
            decomp : str, optional
                ...
                'primary_decomp', 'invariant_factor_decomp', or 'alex_polys_decomp' (default).
...
##########CODE TO FIND ALL POSSIBLE PRIMARY DECOMPOSITIONS############
    ...
    def _get_primary_splitting(fm):
        ...
        raise UnderdeterminedDecompositionError(
            "Primary decomposition is underdetermined: "
            "multiple exponent splittings are consistent with the Fox matrix."
            )
...
def PD_from_dt(dt):
#   Converts  DT code to PD code
    if not SNAPPY_AVAILABLE:
        raise ImportError(
            "SnapPy is required for DT input notation. "
            "Install SnapPy or use notation='PD'."
        )
    dt_str = 'DT: [(%s)]' % ','.join(map(str, dt))
    try: 
        k = snappy.Link(dt_str)
    except Exception as e:
        raise ValueError(f"Invalid DT code {dt}: SnapPy could not construct a link — {e}")
    return k.PD_code()

def PD_from_dt_alphabetical(dt_alphabetical):
#   Converts alphabetic DT code to PD code
    if not SNAPPY_AVAILABLE:
        raise ImportError(
            "SnapPy is required for DT input notation. "
            "Install SnapPy or use notation='PD'."
    ...
    dt_notation = []
    for ch in dt_alphabetical:
        if ch.islower():
            dt_notation.append(2 * (ord(ch) - ord('a') + 1))
        else:
            dt_notation.append(-2 * (ord(ch) - ord('A') + 1))

    dt_str = 'DT: [(%s)]' % ','.join(map(str, dt_notation))
    try:
        k = snappy.Link(dt_str)
    except Exception as e:
         raise ValueError(f"Invalid alphabetical DT code '{dt_alphabetical}': SnapPy could not construct a link — {e}")
    return k.PD_code()